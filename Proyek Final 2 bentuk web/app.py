...
def index():
    if request.method == 'POST':
        ...
        if physical_port is None:
            flash(f"Physical_OutputPort untuk {logical_port} tidak ditemukan.")
            return redirect(url_for('index'))

        # Find the corresponding CSV file
        file_path = find_csv_file('TEMP_OUTPUT_PWR240624', physical_port, subchannel)
        if file_path is None:
            flash(f"File CSV untuk {physical_port}_{subchannel} tidak ditemukan.")
            return redirect(url_for('index'))
        ...
        flash("Data berhasil diproses dan plot telah ditampilkan.")
        return redirect(url_for('index'))
